...
const defaultConfig = {
  ...
  {
    name   : "that nginx log path exists",
    cmd    : 'ensure',
    ensure : [{
      cmd       : "ssh",
      ssh       : `test -e :nginxLogPath && echo "OK" || echo "NOK"`,
      is        : "OK",
      transform : [ "trim" ]
    }],
    commands : [
    ...
    name   : "that nginx config path exists",
    cmd    : 'ensure',
    ensure : [{
      cmd       : "ssh",
      ssh       : `test -e :nginxConfPath && echo "OK" || echo "NOK"`,
      is        : "OK",
      transform : [ "trim" ]
    }],
    commands : [
  ...
  "APP_UPLOAD": [{
    "name"   : "ensure the version path exists",
    "cmd"    : "ensure",
    "ensure" : [{
      "cmd" : "ssh",
      "ssh" : ":deploy/exists :versionPath",
      "is"  : "OK",
      "transform" : [ "trim" ]
    }],
    "commands" : [
    ...
    "cmd": "scp",
    "src": ":sourcePath",
    "dest": ":versionPath/:timestamp"
  } ],
  "START_APP": [
    `PORT=:PORT pm2 start :versionPath/:timestamp --name :timestamp`
  ],
  BLUEGREEN : [
    `sed 's/:3000/::PORT/g' < :nginxSourceConfig > :nginxConfPath/nginx-:timestamp.conf`,
    `sudo nginx -s stop && sudo nginx -c :nginxConfPath/nginx-:timestamp.conf`
  ],
  STOP_APP : [
    `:deploystop-other-processes :timestamp`
  ]
}
